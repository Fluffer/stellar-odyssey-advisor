...
//      is strictly better.
//   2. Droid-dodge heuristic (no extracted formula - a player-stated rule of
//      thumb): once average droid maneuverability is high, 'Droids dodge
//      chance' mods are flagged as low value in favor of drop-chance mods.

const {
  ITEM_CATEGORY, BATTLING_NPCS, SHIP_ITEM_RARITY_MULT, ITEM_MATCHING_SKILL,
} = require("./constants.js");

const DROID_DODGE_HEURISTIC_THRESHOLD = 50;
...
function planShipItems(state) {
  ...
  // --- droid-dodge heuristic input ---
  const droids = state.droids || [];
  const avgManeuverability = droids.length
    ? droids.reduce((sum, dd) => sum + (dd.maneuverability || 0), 0) / droids.length
    : 0;
  ...
  for (const slot of Object.keys(ITEM_CATEGORY)) {
    ...
    if (slot === "laser_slot" || slot === "probes_slot") {
      if (avgManeuverability >= DROID_DODGE_HEURISTIC_THRESHOLD && bonuses.includes("Droids dodge chance")) {
        recommendations.push(
          `Heuristic: avg droid maneuverability ${Math.round(avgManeuverability * 10) / 10}% >= ${DROID_DODGE_HEURISTIC_THRESHOLD}% — 'Droids dodge chance' is likely low value now; prefer 'Rare Resource drop chance'`
        );
      }
    }
    if (item.anomaly) {
  ...
  return { items, cooldown, scan };
}

module.exports = {
  planShipItems, valueMaxForLevel, cooldownFloorBreakpoint, cooldownAt,
  nonDamageModValue, damageModValue, DROID_DODGE_HEURISTIC_THRESHOLD,
};
