...
function makeBattleValidator(state) {
  ...
  return {
    baseLevels,
    note: `Battle check: every default-tab change is simulated at your current max NPC level @ ${WR_THRESHOLD}%+ winrate (${CHECK_RUNS} runs, squadron boost +${ssBoost}%); a change that lowers any NPC's max level is rejected.`,
    touchesBattle,
