...
const ACTIVITY_RELEVANT_STATS = {
  ...
  //
  // Fitting the game's own voyager formula term by term against that
  // expedition -- (star + bodies + distance) * (1 + voyagerReward/100)
  // * (1 + owl + baseBodyType + stationBonus + exploringPvPTiles)/100
  // * voyagerTechBonus * Premium -- the only combination within 0.5% is
  // voyagerReward = 0 with owl 14% and ONE of {voyagerTechBonus, Premium}
  // at 1.10 (the two are degenerate at that value). EVERY combination that
  // includes the 20% reward bonus that was already bought lands 4.4% or
  // more out, and the closest one only gets there by switching off both the
  // Owl and the tech skill. So the reward bonus is not visible in this
  // result. Whether it never applies to expedition dust, or the reward is
  // computed from a player snapshot taken when the expedition launches, one
  // expedition cannot say -- rerunning one now, well after the upgrades,
  // separates them: 1.14 * 1.10 * 1.20 * 1.10 predicts ~29,800 gross for a
  // comparable system against the ~22,600 a no-lever model predicts.
