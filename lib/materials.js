...
// Hourly base production per material name, or {} without a founded base.
function baseRates(state) {
  ...
  return BaseMath.productionByProduct(base, eff);
}
...
function planMaterials(state) {
  ...
  // is in stock (so lab output no blueprint consumes is still listed).
  // Zero-stock, zero-need names are left out as noise.
  ...
  let deficitCount = 0;
  const rates = baseRates(state);
  // Base modules make some of these every hour: the rate and how long the
  // deficit takes to close on its own.
  const withBase = (entry) => {
    const rate = rates[entry.material] || 0;
    entry.basePerHour = rate;
    entry.hoursToCover = rate > 0 && entry.deficit > 0 ? entry.deficit / rate : null;
    return entry;
  };
  ...
  for (const src of [state.commonResources, state.rareCurrencies]) {
    ...
    for (const [k, v] of Object.entries(src)) {
      ...
      gathered.push(withBase({ material, stock, neededAllUses, deficit, source: "gathered" }));
    }
  }
  for (const material of names) {
    ...
    const entry = withBase({ material, stock, neededAllUses, deficit, source });
    if (source === "npc") {
