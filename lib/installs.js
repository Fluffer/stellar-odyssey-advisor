...
const {
  ITEM_CATEGORY, ITEM_ACTIVITIES, activityChain, STAT_CAPS, STAT_CAPS_BY_CONTEXT, SLOTS_PER_GROUP, MAX_SAME_STAT,
...
function planInstalls(ship, pool, opts = {}) {
  ...
  const runPass = (actFilter, poolFilter, useWeights) => {
    ...
    while (guard++ < 500) {
      ...
      for (const slot of Object.keys(work)) {
        ...
        for (const act of ITEM_ACTIVITIES[slot]) {
          ...
          for (const cand of pool) {
            ...
            // poolFilter may answer "same": the stat is outside this
            // variant's strategy, but upgrading a catalyst of that stat that
            // is ALREADY installed changes nothing about the strategy, so a
            // same-stat replacement stays allowed (free-slot installs and
            // cross-stat replacements do not).
            const allowed = poolFilter ? poolFilter(cand, act, slot) : true;
            if (!allowed) continue;
            const sameStatOnly = allowed === "same";
            if (statCategory(cand.stat) !== cat) continue;
            const ctxKey = act === "default" ? "default" : act;
            // Context-aware, like the display path at :227 and the warnings in
            // advisor-core.js:150. `block` has its cap ONLY in
            // STAT_CAPS_BY_CONTEXT (40 normally, 25 in galaxy boss), so a bare
            // STAT_CAPS lookup left it undefined and every headroom guard
            // below was skipped -- the planner kept recommending block
            // catalysts while the warnings tab said the cap was already blown.
            const cap = (STAT_CAPS_BY_CONTEXT[cand.stat] || {})[ctxKey] ?? STAT_CAPS[cand.stat];
            ...
            if (group.length < SLOTS_PER_GROUP && !sameStatOnly) {
            ...
            // Replacements normally only when the group is full; a same-stat
            // upgrade may also replace its weaker sibling with free slots left.
            if (group.length >= SLOTS_PER_GROUP || sameStatOnly) {
              for (const m of group) {
                if (sameStatOnly && m.stat !== cand.stat) continue;
                const afterCount = group.filter(x => x !== m && x.stat === cand.stat).length;
                if (afterCount >= MAX_SAME_STAT) continue;
                const removedEff = catalystValue(m);
                // Candidate's own effective value once m is gone (halved if
                // another same-stat catalyst remains). Cap headroom is taken
                // against the context total WITHOUT m's contribution when m
                // shares the candidate's stat (it leaves before we enter).
                let candEff = groupValueWith(group, cand, m) - (before - removedEff);
                if (cap !== undefined) {
                  const sameRemoved = m.stat === cand.stat ? removedEff : 0;
                  candEff = Math.min(candEff, cap - (curTotal - sameRemoved));
                }
                const gain = candEff - removedEff;
                if (gain < MIN_WORTHWHILE_GAIN) continue;
                // A same-stat upgrade is strategy-neutral: never let a
                // variant's low weight for that stat suppress it.
                const weight = weights[cand.stat] !== undefined ? weights[cand.stat] : 1;
                const ranked = gain * (m.stat === cand.stat ? Math.max(1, weight) : weight);
                if (ranked > 0.01 && (!best || ranked > best.ranked)) {
                  best = {
                    gain, ranked, slot, act, cand, remove: m,
                    rawCandVal: candEff, isEmptyGroup: false, inheritedFullStats: null,
                  };
                }
              }
            }
      ...
      // Cap context for the GUI: what the stat totals in this activity
      // before and after the action, against the activity-specific cap
      // (block: 40 by default, 25 in galaxy boss).
      const displayCap = (STAT_CAPS_BY_CONTEXT[best.cand.stat] || {})[ctxKey] ?? STAT_CAPS[best.cand.stat];
      const capTotalBefore = totals[ctxKey][best.cand.stat] || 0;
      // Same-stat count AFTER the outgoing catalyst (if any) has left the
      // group - a replace that swaps out the only other same-stat catalyst
      // leaves the newcomer unhalved.
      const sameAfter = group.filter(x => x !== best.remove && x.stat === best.cand.stat).length;
      ...
      if (displayCap !== undefined) {
        Object.assign(actions[actions.length - 1], {
          cap: displayCap, capTotalBefore, capTotalAfter: totals[ctxKey][best.cand.stat],
        });
      }
    }
  };
  ...
  runPass(
    ...
    (c, act, slot) => {
      ...
      if (relevant && !relevant.includes(c.stat)) return false;
      // Variant restriction: stats outside the list may only upgrade a
      // catalyst of the same stat that is already installed.
      if (opts.restrictDefault && !opts.restrictDefault.includes(c.stat)) return "same";
      return true;
    },
    true
  );
