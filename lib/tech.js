...
function techBattleRanking(state) {
  ...
  for (const key of TECH_BATTLE_SIM_SKILLS) {
    ...
    const after = maxLevelsOf(p2);
    let total = 0;
    for (const npc of Object.keys(base)) total += after[npc] - base[npc];
    const avgDelta = total / Object.keys(base).length;
  ...
  return { baselineAvg: Math.round(baselineAvg * 10) / 10, rows };
}
