...
const READ_ALL = `(() => {
  ...
  return JSON.stringify({
    ...
    // Squadron Forge and highest Crafting dungeon run: the Craftron set-up
    // panel's Forge and Dungeon bonuses (and the crafting-XP simulator) read
    // them. They live on the squadron object, not the squadronStore.
    squadron: (function () {
      var sq = (user && user.squadron) || (p && p.squadron) || null;
      if (!sq) return null;
      var forge = null;
      var buildings = sq.buildings || [];
      for (var i = 0; i < buildings.length; i++) {
        if (buildings[i] && buildings[i].name === 'Forge') forge = buildings[i];
      }
      var highest = 0;
      try { highest = (sq.dungeons.statistics.craftingRuns.highest) || 0; } catch (e) { highest = 0; }
      return { forgeLevel: forge ? (forge.level || 0) : 0, craftingHighest: highest };
    })(),
    ...
    // Systems carry coordinate_x/y/z; the plane distance to the nearest
    // starter system is the uncapped part of a discovery's dust value.
    currentSystem: (exploreS && exploreS.currentSystem) ? {
      name: exploreS.currentSystem.name, star: exploreS.currentSystem.star,
      x: exploreS.currentSystem.coordinate_x, y: exploreS.currentSystem.coordinate_y, z: exploreS.currentSystem.coordinate_z,
    ...
    bookmarks: (exploreS && Array.isArray(exploreS.bookmarks)) ? exploreS.bookmarks
      ...
      .map(function (b) {
        return {
          name: b.system.name, star: b.system.star,
          x: b.system.coordinate_x, y: b.system.coordinate_y, z: b.system.coordinate_z,
          bodies: (b.system.bodies || []).map(function (x) { return x.type; }),
        };
      }) : [],
...
// Fallback when PowerShell is blocked (locked-down / non-English Windows):
// `netstat -ano` lists listening TCP rows with remote 0.0.0.0:0 / [::]:0, and
// `tasklist` maps the PID to a process name. Neither needs PowerShell, and
// both are read by column position, so a localized Windows still parses.
function listListenersNetstat() {
  const tcp = execSync("netstat -ano -p tcp", { encoding: "utf8", timeout: 15000, windowsHide: true });
  const seen = new Set();
  const rows = [];
  for (const raw of tcp.split(/\r?\n/)) {
    const parts = raw.trim().split(/\s+/);
    if (parts.length < 5) continue;
    const local = parts[1];
    const remote = parts[2];
    const pid = parts[4];
    if (!/^(0\.0\.0\.0:0|\[::\]:0|\[::1\]:0)$/.test(remote)) continue;
    const m = /:(\d+)$/.exec(local);
    if (!m) continue;
    const port = parseInt(m[1], 10);
    if (!port || seen.has(port)) continue;
    seen.add(port);
    rows.push({ port, pid, name: "", hasWindow: false, path: "" });
  }
  if (!rows.length) return rows;
  try {
    const tl = execSync("tasklist /fo csv /nh", { encoding: "utf8", timeout: 15000, windowsHide: true });
    const nameByPid = new Map();
    for (const line of tl.split(/\r?\n/)) {
      const m = /^"([^"]*)","(\d+)"/.exec(line.trim());
      if (m) nameByPid.set(m[2], m[1].replace(/\.exe$/i, ""));
    }
    for (const row of rows) row.name = nameByPid.get(row.pid) || "";
  } catch {}
  return rows;
}

// Enumerate listening ports, trying PowerShell first and netstat second, and
// keep the error text so diagnose-connection.js can say WHY the machine
// could not be scanned instead of just "no ports".
function listListenersDetailed() {
  let psError;
  try {
    const rows = parsePsRows(execSync(PS_LISTENERS, { encoding: "utf8", timeout: 15000, windowsHide: true }));
    if (rows.length) return { rows, method: "powershell", error: null };
    psError = "PowerShell returned no listening ports";
  } catch (e) {
    psError = "PowerShell failed: " + (e && e.message ? e.message : String(e));
  }
  try {
    const rows = listListenersNetstat();
    if (rows.length) return { rows, method: "netstat", error: null };
    return { rows: [], method: null, error: psError + "; netstat returned no listening ports" };
  } catch (e) {
    return { rows: [], method: null, error: psError + "; netstat failed: " + (e && e.message ? e.message : String(e)) };
  }
}

function listListeners() {
  return listListenersDetailed().rows;
}

// --- DevToolsActivePort ----------------------------------------------------
// Chromium/Electron writes the port it picked for --remote-debugging-port=0
// into <userDataDir>/DevToolsActivePort (first line: the port). The game
// launches itself with exactly that switch (electron-main.js), so this file
// is the most direct way to find its socket -- no PowerShell, no process
// enumeration, no dependency on the Windows UI language. userDataDir is
// %APPDATA%\<productName> ("Stellar Odyssey Game"), but a repack or a
// differently-named build is still found by scanning one level under
// %APPDATA%/%LOCALAPPDATA% for any DevToolsActivePort file and confirming the
// page with pageIsGame().
const DEVTOOLS_FILE = "DevToolsActivePort";
const USERDATA_NAMES = ["Stellar Odyssey Game", "stellarodyssey", "Stellar Odyssey", "stellar-odyssey"];

function devtoolsPorts() {
  const dirs = new Set();
  const bases = [process.env.APPDATA, process.env.LOCALAPPDATA].filter(Boolean);
  for (const base of bases) {
    for (const name of USERDATA_NAMES) dirs.add(path.join(base, name));
    try {
  ...
  for (const dir of dirs) {
    try {
      const first = String(fs.readFileSync(path.join(dir, DEVTOOLS_FILE), "utf8")).split(/\r?\n/)[0].trim();
...
async function discoverGame() {
  // 1. The game's own DevToolsActivePort. This needs no PowerShell, no
  //    process enumeration and no English Windows, so it is tried first.
  let sawCdp = false;
  ...
  // 2. Enumerate listening ports (PowerShell first, netstat second).
  const scan = listListenersDetailed();
  const rows = scan.rows;
  if (!rows.length && !sawCdp) {
    return { url: null, reason: "could not enumerate listening ports and no DevToolsActivePort was found (" + (scan.error || "PowerShell/Get-NetTCPConnection blocked?") + ")" };
  }
...
module.exports = {
  ...
  listListeners, listListenersDetailed, tierOf, pageIsGame, devtoolsPorts,
};
