...
const { FOUNDING_BUNDLE, MATERIAL_BUILDINGS, normalizeBase, productionByProduct } = require("../public/base-math.js");
...
function planLab(state, opts) {
  ...
  return {
    ...
    // What the founded base makes per hour, by product (raw resources the
    // Resource miner mines, materials, cores).
    baseProduction: productionByProduct(normalizeBase(state.base), (state.player && state.player.skills && state.player.skills.base_module_efficiency_boost) || 0),
  };
}
