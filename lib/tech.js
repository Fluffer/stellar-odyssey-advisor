...
function techBattleRanking(state) {
  ...
  // Ranking metric: WINRATE DELTA AT A FIXED LEVEL, not the difference of two
  // max-level searches.
  //
  // maxLevelAtWinrate() reseeds per level (advisor-battle.js:215), so its
  // result is not monotone in level and two searches differ by tens of levels
  // on identical builds -- differencing them measured noise, not skill value,
  // and the sign flipped between analyses at unchanged skill levels. Holding
  // the level fixed makes both builds run the SAME rng stream (common random
  // numbers), so the comparison is clean and continuous.
  //
  ...
  // reported in the levels the tab has always spoken in. Measured on the same
  // streams, symmetrically around each probe level.
  const slopePerLevel = (() => {
    let sum = 0, n = 0;
    for (const npc of BATTLING_NPCS) {
  ...
  for (const key of TECH_BATTLE_SIM_SKILLS) {
    ...
    // Equivalent NPC levels, so the column keeps its old meaning.
