...
// Skills the game itself lists as locked (the client bundle's LOCKED_SKILLS:
// the five dungeon skills while solo and squadron dungeons are locked on the
// Steam realm). A skill the player has already levelled is never locked,
...
const TECH_SKILLS = [
  ...
  { key: "base_module_efficiency_boost", cat: "base", desc: "Base module efficiency +1% per level" },
...
function techBattleRanking(state) {
  ...
  const probeLevelFor = (npc) => {
    let lo = 1, hi = Math.max(2, Math.round(base[npc.name] * 1.6)), mid;
...
// Quantum-core income the way the game's Technology page states it:
// battling drops min(18, 8 + floor(battling level / 6)) per hour, plus the
// Quantum server's hourly tick when the base has one running (guaranteed
// amount, and a (boost mod 100)% chance of one more). The daily bonus is a
// user-tuned default.
function coreIncome(state) {
  const lvl = state.skillLevels && state.skillLevels.battling != null ? Number(state.skillLevels.battling) : null;
  const battlingPerHour = lvl == null ? null : Math.min(18, 8 + Math.floor(lvl / 6));
  const skills = (state.player && state.player.skills) || {};
  const eff = skills.base_module_efficiency_boost || 0;
  const base = BaseMath.normalizeBase(state.base);
  const qs = base ? base.modules.find(m => m.name === "Quantum server" && m.unlocked && m.active) : null;
  const quantumServer = qs ? {
    level: qs.level,
    guaranteedPerHour: BaseMath.guaranteedOutputPerTick(qs, eff),
    extraChance: BaseMath.extraDropChance(qs, eff),
    expectedPerHour: BaseMath.expectedOutputPerTick(qs, eff),
  } : null;
  const ratePerHour = (battlingPerHour == null ? 18 : battlingPerHour) + (quantumServer ? quantumServer.expectedPerHour : 0);
  return { battlingLevel: lvl, battlingPerHour, quantumServer, ratePerHour };
}
...
function planTech(state) {
  ...
  const rows = TECH_SKILLS.map(s => {
    ...
    const locked = TECH_LOCKED_SKILLS.has(s.key) && level === 0;
  ...
  // the cores on hand. The hourly rate comes from coreIncome; the daily
  // bonus is a default. The GUI lets the user tune both.
  const income = coreIncome(state);
  const coresAssigned = rows.reduce((sum, r) => sum + (r.locked ? 0 : r.cumulativeSpent), 0);
  const coresToMaxAll = rows.reduce((sum, r) => sum + (r.coresToMax || 0), 0);
  // What it takes to fully max every unlocked skill: cores already assigned
  // (cumulative spend at the current levels) plus the cores still remaining.
  const coresMaxTotal = coresAssigned + coresToMaxAll;
  ...
  const maxOut = {
    ...
    defaultRatePerHour: Math.round(income.ratePerHour * 10) / 10,
...
module.exports = {
  ...
  techSkillCostNext, techBattleRanking, coreIncome, planTech,
};
