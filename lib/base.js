...
function buildBaseInput(state, opts) {
  ...
  } catch { upkeepReduction = 0; } // a ship shape the item scorer chokes on must not break base planning
  ...
  // Once founded, the rate the server stored on the base replaces the star
  // selector (it only feeds the unconfirmed footnote variant anyway).
  const starRate = live && live.stellariumHourly ? live.stellariumHourly : starInfo(chosenStar).rate;
  return {
    modules, founded: !!live, stellarium: live ? live.stellarium : 0,
    levels: opts.levels || {},
    starName: chosenStar, starRate,
...
function planBaseFromState(state, opts) {
  ...
  // Moot once the base exists: null so no consumer can show the bundle as a need.
  const founding = live ? null : { bundle, ready: bundle.every(b => b.have >= b.units) };

  const cur = state.currentSystem || {};
  const current = { name: cur.name || null, ...starInfo(cur.star), ...Systems.describe(cur, cur) };
  const bookmarks = (state.bookmarks || []).map(b => ({ name: b.name || null, ...starInfo(b.star), ...Systems.describe(b, cur) }));
  ...
  if (live) {
    ...
    const modules = live.modules.map(m => ({
      ...
      guaranteed: BaseMath.guaranteedOutputPerTick(m, eff), extraChance: BaseMath.extraDropChance(m, eff),
      tickHours: BaseMath.tickHours(m, live.stellariumHourly), selection: m.selection || [],
    ...
    const minerLive = live.modules.find(m => m.name === "Stellarium miner");
    const { drops, etaTick } = BaseMath.unlockEta(next ? next.cost : 0, live.stellarium, minerLive && minerLive.unlocked ? BaseMath.moduleBoost(minerLive, eff) : 0, live.nextStellariumTick);
    liveBlock = {
      name: live.name, stellarium: live.stellarium, nextStellariumTick: live.nextStellariumTick, modules,
      // The body the base was founded on (nodeType) and the +10% XP it grants.
      bodyType: live.bodyType, bodyActivity: BaseMath.BODY_NODE_BONUSES[live.bodyType] || null,
      stellariumHourly: live.stellariumHourly || null,
      production: BaseMath.baseProduction(live, eff),
      nextUnlock: next ? { name: next.name, cost: next.cost, etaDays: next.cost <= live.stellarium ? 0 : (perDay > 0 ? (next.cost - live.stellarium) / perDay : Infinity), drops, etaTick, estimate: true } : null,
    };
  }

  // Stellarium facts for the GUI's Stellarium section: the game's drop rate
  // for the miner, how often it drops, what unlocks and tiers cost, and the
  // star table the client ships but never reads (reference only). The star
  // variant is the unconfirmed footnote; see ESTIMATES in public/base-math.js.
  const ticksPerDay = 24 / BaseMath.ESTIMATES.STELLARIUM_TICK_HOURS;
  const minerMod = input.modules.find(m => m.name === "Stellarium miner");
  const minerBoost = minerMod && minerMod.unlocked ? BaseMath.moduleBoost(minerMod, input.efficiencyBoost) : 0;
  const unlockedCount = live ? live.modules.filter(m => m.unlocked).length : 0;
  const unlockCurve = [];
  for (let n = Math.max(1, unlockedCount); n < Math.max(1, unlockedCount) + 8; n++) unlockCurve.push({ unlocked: n, cost: BaseMath.unlockCost(n) });
  const tierCurve = [1, 10, 50, 100, 150, 200].map(tier => ({ tier, cost: BaseMath.tierCost(tier - 1) }));
  const stellarium = {
    tickHours: BaseMath.ESTIMATES.STELLARIUM_TICK_HOURS, ticksPerDay,
    perTick: plan.stellariumPerDay / ticksPerDay, dropRate: plan.dropRate,
    perDay: plan.stellariumPerDay,
    // Unconfirmed server-side variant: the rate the server stores on the
    // founded base (else the chosen star's table rate) times the drop.
    storedRate: live && live.stellariumHourly ? live.stellariumHourly : null,
    perTickStar: plan.stellariumPerDayStar / ticksPerDay, perDayStar: plan.stellariumPerDayStar,
    starRate: input.starRate, minerBoost,
    // Drops actually seen in the history log; the server fills this in
    // (observedStellariumDrops in lib/income.js). Null until one is seen.
    observed: null,
    held: live ? live.stellarium : 0,
    nextTick: live ? (live.nextStellariumTick || null) : null,
    unlockedCount, unlockCurve, tierCurve,
    stars: Object.keys(BaseMath.STAR_BONUSES).map(name => ({ name, ...BaseMath.STAR_BONUSES[name] })),
    lockedModules: plan.unlocks.filter(u => !u.unlocked),
    totalLeft: plan.totalStellariumLeft,
  };
  ...
  return {
    ...
    founding, location, stellarium, input, plan, live: liveBlock, labPanelHint,
