    ...
    if (slot === "engine_slot" || slot === "sensors_slot") {
      if (!cooldownModPresent) {
        if (!withCooldownModAt10ly) {
          recommendations.push(
            `Cooldown is at the 5-min floor at 10 ly even without the global boost (${cooldown.dBase}% >= ${breakpointD10ly}% breakpoint) — a cooldown mod would be wasted; prefer 'Scan reward boost'`
          );
        } else if (flooredNowAt10ly) {
          recommendations.push(
            `Cooldown floored up to ~${cooldown.flooredUpToLy} ly with the +${globalBoost.bonus}% global Cooldown boost (tier ${globalBoost.tier}, ~${globalBoost.hoursLeft}h left) — a cooldown mod pays off only once the boost lapses (${cooldown.dBase}% < ${breakpointD10ly}% breakpoint without it)`
          );
        } else {
          recommendations.push(
            `No 'Engine cooldown reduction' mod equipped on engine or sensors — adding one would still help at 10 ly (${cooldown.d}% < ${breakpointD10ly}% floor breakpoint)`
          );
        }
      }
    }
