...
const ACTIVITY_RELEVANT_STATS = {
  ...
  // Boost items (laser/probes) have NO voyager tab, so during an expedition
  // they resolve voyager -> exploring and their EXPLORING group applies. The
  // game says so twice: the install dialog labels that tab "Exploring &
  // Voyager" on those two items (client label override
  // { boost: { exploring: 'Exploring & Voyager' } }) and lists its Cosmic
  // Dust Bonus and Exploring XP under "Current bonuses (Exploring &
  // Voyager)". Both stats therefore DO something on an expedition, and since
  // cosmic_dust_bonus is a boost stat it can only ever sit on those two
  // items -- an expedition gets the same dust bonus your ship gets exploring.
  voyager: ["fuel_efficiency", "catalyst_drop_chance", "voyager_jumps_bonus",
            "cosmic_dust_bonus", "exploring_xp"],
};
